...
#define statusWatch_FUNCTION( vNote, vFunction ) void vFunction(void* pRev)

statusWatch_FUNCTION(GetID,FCT_Response_GetID)
{
	u8 tab[20];
	char ch[20] = {'\0'};
...
statusWatch_FUNCTION(GetServerAddr,FCT_Response_GetServerAddr)
{
	char ch[20] = {'\0'};
...
statusWatch_FUNCTION(GetServerPort,FCT_Response_GetServerPort)
{
	u8 Buffer[4];
	u32 portVal;
...
statusWatch_FUNCTION(GetReportInterval,FCT_Response_GetReportInterval)
{
...
statusWatch_FUNCTION(GetSimCardNum,FCT_Response_GetSimCardNum)
{
	char ch[20] = {'\0'};	
	u8 Buffer[20];
...
statusWatch_FUNCTION(GetDeviceInfo,FCT_Response_GetDeviceInfo)
{
...
statusWatch_FUNCTION(EnterFactory,FCT_Response_EnterFactory)
{
...
statusWatch_FUNCTION(SysReset,FCT_Response_SysReset)
{
...
statusWatch_FUNCTION(SetID,FCT_Response_SetID)
{
	char *ch = (char*)pRev;
	u8 buffer[7];
...
statusWatch_FUNCTION(SetAddr,FCT_Response_SetAddr)
{
	char *ch = (char*)pRev;
...
statusWatch_FUNCTION(SetPort,FCT_Response_SetPort)
{
	char *ch = (char*)pRev;
	u32 tmp;
	u8  Buffer[4];
...
statusWatch_FUNCTION(SetUpInterval,FCT_Response_SetUpInterval)
{
	char *ch = (char*)pRev;
	u32 tmp;
	u8  Buffer[4];
...
void FCT_SelfCheckInit(void)
{
	...
	if(0xaa != selfCheckFlg)
	{
		...
		//进入FCT模式
		APP_DEBUG("MODE:FCTMODE\r\n");
	}
	else
	{
		selfCheckFlg = 0;
		APP_DEBUG("MODE:NORMALMODE\r\n");
	}
}
